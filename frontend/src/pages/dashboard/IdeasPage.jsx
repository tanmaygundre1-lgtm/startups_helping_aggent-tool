    ...
    <AppLayout>
      <div style={{ padding: 'var(--space-lg)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 'var(--space-md)' }}>
          ...
          <Button onClick={() => navigate("/app/ideas/create")}>+ Create Idea</Button>
        </div>
        {loading ? <p>Loading ideas...</p> : ideas.length === 0 ? (
          <Card>
            <p>You haven't created a startup idea yet.</p>
            <Button onClick={() => navigate("/app/ideas/create")}>Create your first idea</Button>
          </Card>
        ) : (
            ...
            {ideas.map(idea => (
              <Card key={idea._id}>
                ...
                <div style={{ display: 'flex', gap: 'var(--space-sm)' }}>
                  <Button variant="secondary" onClick={() => navigate(`/app/ideas/${idea._id}`)}>View Idea</Button>
                  <Button variant="secondary" onClick={() => handleDelete(idea._id)}>Delete</Button>
                </div>
              </Card>
            ))}
          </div>
